...
library LibDelegateCall {

    // Function that checks if submittedContract uses the opcode CALL (0xF4), find address it calls, and checks if the address is in a blacklist of addresses
    function checkCall(address submittedContract, address[] memory _blacklist) public view returns (bool) {
        assembly {
            ...
            for { } lt(ptr, end) { } {
                ...
                // If opcode is CALL (0xF4), check if the address it calls is in the blacklist
                if eq(opcode, 0xF4) {
    ...
    // Function that checks if submittedContract uses the opcode CALL (0xF4), then finds the address and function signature related to the CALL and checks them against a blacklist
